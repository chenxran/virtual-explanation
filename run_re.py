...
from utils import construct_virtual_explanation, evaluate_tacred, load_explanation, replace_exp_with_random_tokens
...
class REDataset(Dataset):
    ...
    def insert_entity(self, exp, entities):
        if '<mask>' in exp:
            ...
            exp.replace('{e1}', entities[0])
            exp.replace('{e2}', entities[1])
...
class TACREDDataset(REDataset):
    def __init__(self, args, path, explanations, tokenizer, base_tokenizer_length, label2id):
        super(TACREDDataset, self).__init__(args, path, explanations, tokenizer, base_tokenizer_length)
...
class ExpBERT(nn.Module):
    def __init__(self, args, exp_num):
        ...
        self.linear = nn.Linear(self.config.hidden_size * exp_num, args.num_labels)
...
class Trainer(object):
    def __init__(self, args, seed, checkpoint=None):
        ...
        explanations = construct_virtual_explanation(args.exp_num, args.num_explanation_tokens) if args.explanation else load_explanation(args.task)
        ...
        explanations = replace_exp_with_random_tokens(explanations, self.tokenizer, base_tokenizer_length, args.replace_ratio, args.replace_with_new_token)
        

        if args.task == 'tacred':
            with open('data/tacred/label2id.json', 'r', encoding='utf-8') as file:
                label2id = json.load(file)
            self.train_dataset = TACREDDataset(self.args, TASK2PATH[self.args.trainset], explanations, self.tokenizer, base_tokenizer_length, label2id)
            self.test_dataset = TACREDDataset(self.args, TASK2PATH[self.args.testset], explanations, self.tokenizer, base_tokenizer_length, label2id)
            self.label2id = label2id
        else:
            self.train_dataset = REDataset(self.args, TASK2PATH[self.args.trainset], explanations, self.tokenizer, base_tokenizer_length)
            self.test_dataset = REDataset(self.args, TASK2PATH[self.args.testset], explanations, self.tokenizer, base_tokenizer_length)
